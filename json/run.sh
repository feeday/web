...
if [[ -d /workspace/ComfyUI || -w /workspace ]]; then
  DEFAULT_COMFYUI_DIR=/workspace/ComfyUI
else
...
install_requirements() {
  ...
  # 部分节点的 install.py 使用相对路径，必须从仓库目录执行。
  [[ -f "$dir/install.py" ]] && (cd "$dir" && "$PYTHON" install.py)
}
...
# run.sh 可以被单独复制、改名后执行：优先寻找本地工作流，找不到就从项目站点下载。
if [[ ! -f "$WORKFLOW_FILE" ]]; then
  for candidate in "$PWD/$WORKFLOW_NAME" "/workspace/web/json/$WORKFLOW_NAME"; do
    if [[ -f "$candidate" ]]; then
...
if [[ ! -f "$WORKFLOW_FILE" ]]; then
  WORKFLOW_FILE="${SCRIPT_DIR}/${WORKFLOW_NAME}"
  log "下载内置工作流"
  "$PYTHON_BIN" - "$WORKFLOW_FILE" "$WORKFLOW_NAME" <<'PY'
import sys
import urllib.parse
import urllib.request
destination, name = sys.argv[1:]
url = "https://datxy.com/json/" + urllib.parse.quote(name)
...
# 首次部署才安装 CUDA PyTorch，重复运行不会再次下载数 GB 的 wheel。
...
INSTALLED_WORKFLOW="$WORKFLOW_DIR/$WORKFLOW_NAME"
...
"$PYTHON" - "$WORKFLOW_FILE" "$INSTALLED_WORKFLOW" <<'PY'
import json
import os
import sys
...
log "浏览器打开 Workflows 中的工作流即可运行，无需额外配置 GetNode/SetNode"
cd "$COMFYUI_DIR"
