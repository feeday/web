...
def compress_one_file(source_file: Path, source_dir: Path, out_dir: Path, method: int, overwrite: bool, progress: tqdm) -> tuple[Path, Path, int]:
    ...
    try:
        zip_file_with_progress(source_file, temp_zip, method, progress)
...
def main() -> int:
    ...
    total_input_bytes = sum(file.stat().st_size for file in files)
    with tqdm(total=total_input_bytes, unit="B", unit_scale=True, unit_divisor=1024, desc="压缩进度") as progress:
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = [executor.submit(compress_one_file, file, source_dir, out_dir, method, args.overwrite, progress) for file in files]
