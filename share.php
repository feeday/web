...
const DATA_DIR = __DIR__ . '/.share_data';
...
function loadShare(string $id): ?array
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (is_array($file) && ($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
            ...
            $meta += ['kind' => 'file', 'stored' => $stored, 'name' => mb_substr(basename((string) $file['name']), 0, 180), 'mime' => $mime, 'size' => (int) $file['size']];
        } elseif ($text !== '') {
            if (mb_strlen($text, 'UTF-8') > 50000) throw new RuntimeException('文本不能超过 50,000 字');
...
<?php if ($share): ?>
  ...
  <?php else: $src = '?file=' . urlencode($shareId); ?><div class="preview">
    <?php if (str_starts_with($share['mime'], 'image/')): ?><img src="<?= $src ?>" alt="分享的图像">
    <?php elseif (str_starts_with($share['mime'], 'video/')): ?><video src="<?= $src ?>" controls playsinline></video>
    <?php elseif (str_starts_with($share['mime'], 'audio/')): ?><audio src="<?= $src ?>" controls></audio>
    <?php elseif (str_starts_with($share['mime'], 'text/')): ?><iframe src="<?= $src ?>" style="width:100%;height:360px;border:0;background:white" title="文本预览"></iframe>
